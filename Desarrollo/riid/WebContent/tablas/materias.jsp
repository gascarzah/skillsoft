...
function getGrabar(){
	if (document.f.CODIGO.value == "" || document.f.DESCRIPCION.value == ""  || document.f.CGRUPO.value == ""|| document.f.BESTADOREG.value == ""){
...
function ordena(ord){
	if(ord=="gru") location.href="../servlet/CtrlMant?opt=15&ORDEN=a.dgrupo"; 
	if(ord=="cod") location.href="../servlet/CtrlMant?opt=15&ORDEN=t.cmateria"; 
	if(ord=="des") location.href="../servlet/CtrlMant?opt=15&ORDEN=t.dmateria"; 
	if(ord=="est") location.href="../servlet/CtrlMant?opt=15&ORDEN=t.bestadoreg"; 
}
...
<table align="center" width="100%">
  ...
  <tr>
    <td colspan="3" align="center" class="titulo1">MANTENIMIENTO DE LA TABLA DE MOTIVO PRINCIPAL</td>
  </tr>
</table>
<form name="f1" method="post">
	<table width="80%" border="0" align="center" cellspacing="0">
		<tr class="celdas01">
			<td>CODIGO <input name="COD" type="text" class="celdas01" value="" size="10" maxlength="20">
	...
	<table width="80%"  border="1" align="center" cellspacing="0" bordercolor="#3DB7E4">
		<tr>
			<td>
				<table width="100%" border="1" align="center" cellspacing="0" bordercolor="#DFE6EE" bgcolor="#FFFFFF">
					<tr bgcolor="#3DB7E4" class="Estilo3" align="center">
					  ...
					  <td colspan="3">MOTIVO PRINCIPAL</td>
				  </tr>
					<tr bgcolor="#3DB7E4" class="Estilo3" align="center">
					  <!--  <td>ESTADO</td>-->
					  <td colspan="2"><a href="javascript:ordena('gru');" class="a" title="Ordenar por este campo">DESCRIPCION</a></td> 
						...
						<td><a href="javascript:ordena('est');" class="a" title="Ordenar por este campo">ESTADO</a></td>
					</tr>
	<%
			int counter = 0;
			boolean bFil = true;
	...
	%>
					<tr bgcolor="<%=bFil?"#EAEFF5":"#FFFFFF"%>" class="celdas01">
					  <td align="center">
					  <% if (clusuario.getPerfil().compareTo("09")==0 || clusuario.getPerfil().compareTo("00")==0){ %>
              <a href="javascript:getReg('<%=v.get(0)%>','<%=v.get(1)%>','<%=v.get(2)%>','<%=v.get(4)%>','<%=v.get(5)%>');"><img src="../images/fleder.jpg" alt="Seleccione para actualizar datos del registro" width="12" height="12" border="0"></a>
						<% } %>&nbsp;&nbsp;
				    <%= counter + ((Integer.parseInt(objLst.getPag()) - 1)  * 15) %></td> 
						<!--  <td align="center"><%= v.get(5)!=null?v.get(5):"" %></td>-->
						<td align="center" colspan="2"><%= v.get(3)!=null?v.get(3):"" %></td>
		...
		<tr>
			<td>
				<table width="100%" border="1" align="center"  cellspacing="0" bordercolor="#DFE6EE" bgcolor="#FFFFFF">
					...
					<tr align="center">
					  <td><select name="CGRUPO" class="celdas01">
					    <option value="">SELECCIONA</option>
		<%
      v =  (Vector) request.getAttribute ("vGrupos");
      if (v != null) {  
        for (int i = 0; i < v.size(); i++) {  
          vCols = (Vector) v.get(i);  
    %>
					    <option value="<%=vCols.get(0)%>" <%= Util.jnvl((String)request.getParameter("CGRUPO"),"").compareTo(vCols.get(1))==0?"SELECTED":"" %>><%= vCols.get(1)%></option>
		<%
        }
      }
    %>
						...
						<td><input name="DESCRIPCION" type="text" class="celdas01" onfocus="nextfield ='PLAZO';" onchange="cambia(this);" size="100" maxlength="100" /></td>
						<td>
