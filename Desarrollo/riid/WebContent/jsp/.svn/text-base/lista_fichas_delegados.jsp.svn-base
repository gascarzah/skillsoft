...
<%
  ...
  Ficha_t  objLst = (Ficha_t )request.getAttribute("objLst");
	...
	String cdelegado = StringUtils.defaultString((String)request.getAttribute("cdelegado"));
	String ccas = StringUtils.defaultString((String)request.getAttribute("ccas"));
	Vector v;
	Vector vCols;
%>
...
function exportar(){
	...
	location.href='../servlet/CtrlFicha?opt=7&FECHA1='+document.f.FECHA1.value+'&CCAS='+document.f.CCAS.value+'&CDELEGADO='+document.f.CDELEGADO.value;
}
...
<table width="70%"  border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <tr>
      <td align="center" class="titulo1">BANDEJA DE FICHAS DIARIAS POR DELEGADO</td>
      </tr>
    <tr>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td><table width="100%" border="0" cellpadding="0" cellspacing="0"  bgcolor="#FFFFFF" align="center">
            ...
            <tr>
              ...
              <td>Unidad Orgánica 2</td>
              <td><select name="CCAS" class="celdas01" onfocus="nextfield ='CDELEGADO';" onchange="pasa('CDELASIG',this.form.CCAS.options[this.form.CCAS.selectedIndex].value);">
              ...
              <%
							...
							%>
									<option value="<%=vCols.get(0)%>" <%=StringUtils.equals((String)vCols.get(0), ccas)?"selected":"" %> ><%= vCols.get(1)%> </option>
									<%
									}
								}
							%>
              ...
              <td>DELEGADO</td>
              <td><select name="CDELEGADO" class="celdas01">
              <option value="" selected="selected">SELECCIONA</option>
              <%
								v =  (Vector) request.getAttribute ("vDelegados");
								if (v != null) {  
									for (int i = 0; i < v.size(); i++) {  
										vCols = (Vector) v.get(i);  
							%>
									<option value="<%=vCols.get(0)%>" <%=StringUtils.equals((String)vCols.get(0), cdelegado)?"selected":"" %> ><%= vCols.get(0) + " - " +vCols.get(1)%> </option>
									<%
									}
								}
							%>
  ...
  <table  border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <!-- 
      <td align="center">
      <input type="button" name="Submit2" class="boton001" value="Exportar" onclick="exportar();">
      </td>
     -->
    </tr>
  </table>
   ...
   <tr>
     <td>
         ...
         <tr bgcolor="#B0C4DE" class="celdas01" align="center"> 
           ...
           <td bgcolor="#3DB7E4" class="Estilo3">ESTADO DEL REGISTRlO </td>
          </tr>
<%
			int counter = 0;
