...
function inicio(){
	...
	document.f.CODAREA.value = "";
	document.f.CODSERVICIO.value = "";
	document.f.DESSERVICIO.value = "";
...
function getReg(cod,serv,des,est){
	document.f.CODAREA.value = cod;
	document.f.CODSERVICIO.value = serv;
	document.f.DESSERVICIO.value = des;
	...
	eval('document.f.CODAREA.readOnly=true');
	eval('document.f.CODSERVICIO.readOnly=true');
	document.f.CODAREA.style.backgroundColor="#DFE6EE";
}
...
function getSalir(){
	...
	if (caso == 'CANCELAR'){
		...
		eval('document.f.CODAREA.readOnly=false');
		eval('document.f.CODSERVICIO.readOnly=false');
		document.f.CODAREA.style.backgroundColor="#FFFFFF";
		inicio();
	}
	else{
...
function getGrabar(){
	if (document.f.CODAREA.value == "" || document.f.CODSERVICIO.value == ""  || document.f.DESSERVICIO.value == "" || document.f.BESTADOREG.value == ""){
		alert("Por favor ingrese todos los datos");
		document.f.CODAREA.focus();
		return;
	}
...
<table width="100%" align="center">
  ...
  <tr>
    <td colspan="3" align="center" class="titulo1"><%=Constantes.TITULO_MANTENIMIENTO_TABLA_SERVICIO %></td>
  </tr>
</table>
<form name="f1" method="post">
	<table width="700" border="0" align="center" cellspacing="0">
		<tr>
				...
				<td>Código <%= Constantes.TIPO_SERVICIO %><b>*</b>
				<input name="COD" type="text" class="celdas01" value="" onfocus="nextfield ='DES';" size="20" maxlength="20">
               </td>
            <td>
			...
			<input type="hidden" name="COD_PAG" value="<%=objLst.getCodSolicitud()!=null?objLst.getCodSolicitud():"" %>">
			<tr>
			<td></td>
			</tr>
			<tr>
	...
	%>
              ...
              <a href="javascript:getReg('<%=v.get(0)%>','<%=v.get(2)%>','<%=v.get(3)%>','<%=v.get(4)%>');">
		...
		<tr>
			<td>
				<table width="100%" border="1" align="center"  cellspacing="0" bordercolor="#DFE6EE" bgcolor="#FFFFFF">
					...
					<tr align="center">
						<td>
						<select name="CODAREA" class="celdas01" onfocus="nextfield ='DES';" >
                <option value="">SELECCIONAR</option>
                <%
						v =  (Vector) request.getAttribute ("vListArea");
						if (v != null) {  
					...
					%>
						...
						<td><input name="CODSERVICIO" type="text" class="celdas01" onfocus="nextfield ='BESTADOREG';" onchange="cambia(this);" size="50" maxlength="3" ></td>
						<td><input name="DESSERVICIO" type="text" class="celdas01" onfocus="nextfield ='BESTADOREG';" onchange="cambia(this);" size="50" maxlength="60"></td>
						<td>
